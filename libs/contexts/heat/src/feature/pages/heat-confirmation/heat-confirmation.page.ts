import { CommonModule } from '@angular/common';
import { Component, computed, inject, input, signal } from '@angular/core';
import { Router } from '@angular/router';
import {
  AthleteCardComponent,
  TabOption,
  TabSwitcherComponent,
  WodInfoCardComponent,
} from '@hero/ui';
import {
  type HeatConfirmationAthlete,
  type HeatConfirmationHeat,
} from '../../../domain/heat-confirmation.model';
import { type HeatConfirmationPayload } from '../../../infrastructure/heat.repository';

@Component({
  selector: 'app-heat-confirmation',
  standalone: true,
  imports: [
    CommonModule,
    AthleteCardComponent,
    TabSwitcherComponent,
    WodInfoCardComponent,
  ],
  template: `
    <div
      class="relative flex flex-col h-screen bg-slate-50 dark:bg-slate-950 overflow-hidden"
    >
      @if (heat(); as h) {
      <!-- Header -->
      <header
        class="sticky top-0 z-20 flex items-center justify-between px-4 py-4 bg-white/80 dark:bg-slate-900/80 backdrop-blur-[20px] border-b border-slate-200 dark:border-slate-800"
      >
        <div class="flex items-center gap-3">
          <button
            class="text-slate-900 dark:text-white p-1 -ml-1 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
            (click)="onBack()"
            aria-label="Volver"
          >
            <svg
              class="w-7 h-7"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              stroke-width="2.5"
              stroke-linecap="round"
              stroke-linejoin="round"
            >
              <path d="M15 18l-6-6 6-6" />
            </svg>
          </button>
          <div class="flex flex-col">
            <h1
              class="text-slate-900 dark:text-white text-lg font-extrabold leading-tight"
            >
              {{ h.code }}
            </h1>
          </div>
        </div>
        <span
          class="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-3 py-1 rounded-full text-xs font-bold border border-slate-200 dark:border-slate-700"
        >
          {{ h.totalAthletes }} ATLETAS
        </span>
      </header>

      <!-- Scrollable content -->
      <main class="flex-1 overflow-y-auto overflow-x-hidden pb-20">
        <!-- WOD Info Card -->
        <div class="p-4">
          <lib-wod-info-card
            [heatCode]="h.code"
            [wodName]="h.wodName"
            [category]="h.category"
            [startTime]="h.startTime"
            [timeCap]="h.timeCap"
          />
        </div>

        <!-- Tab Switcher + Search -->
        <div class="px-4 space-y-4">
          <lib-tab-switcher
            [tabs]="tabs"
            [activeValue]="'teams'"
          />
          <div class="relative">
            <svg
              class="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 dark:text-slate-500"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              stroke-width="2"
              stroke-linecap="round"
              stroke-linejoin="round"
            >
              <circle cx="11" cy="11" r="8" />
              <path d="m21 21-4.35-4.35" />
            </svg>
            <input
              class="w-full pl-11 pr-4 py-3.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 shadow-sm transition-all outline-none"
              placeholder="Buscar atleta por nombre o dorsal..."
              type="text"
              [value]="searchQuery()"
              (input)="onSearch($event)"
            />
          </div>
        </div>

        <!-- Athlete List grouped by category -->
        <div class="px-4 mt-8 space-y-6">
          @if (groupedAthletes().length === 0) {
          <div
            class="flex flex-col items-center justify-center py-16 text-slate-400 dark:text-slate-500"
          >
            <svg
              class="w-12 h-12 mb-3 opacity-40"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              stroke-width="1.5"
            >
              <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" />
              <circle cx="9" cy="7" r="4" />
              <path d="M23 21v-2a4 4 0 0 0-3-3.87" />
              <path d="M16 3.13a4 4 0 0 1 0 7.75" />
            </svg>
            <p class="text-sm font-medium text-slate-400 dark:text-slate-500">
              No se encontraron atletas
            </p>
          </div>
          } @for (group of groupedAthletes(); track group.label) {
          <section>
            <h3
              class="text-[11px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest px-1 mb-3"
            >
              {{ group.label }}
            </h3>
            <div class="space-y-3">
              @for (athlete of group.athletes; track athlete.id) {
              <lib-athlete-card
                [id]="athlete.id"
                [name]="athlete.name"
                [bibNumber]="athlete.bibNumber"
                [categoryLabel]="athlete.categoryLabel"
                [categoryDetail]="athlete.categoryDetail"
                [type]="athlete.type"
                [scored]="athlete.scored"
                [avatarUrl]="athlete.avatarUrl ?? ''"
                [teamMembers]="getTeamMembers(athlete)"
                (cardClick)="onAthleteClick(athlete)"
              />
              }
            </div>
          </section>
          }
        </div>
      </main>

      <!-- Footer -->
      @if (judge().name) {
      <footer
        class="sticky bottom-0 z-20 flex items-center justify-center px-4 py-3 bg-white/80 dark:bg-slate-900/80 backdrop-blur-[20px] border-t border-slate-200 dark:border-slate-800"
      >
        <span
          class="text-sm font-medium text-slate-500 dark:text-slate-400"
        >
          Juez: {{ judge().name }}
        </span>
      </footer>
      }
      }
      <!-- end @if heat() -->
    </div>
  `,
  styles: [
    `
      :host {
        display: block;
        height: 100dvh;
      }
    `,
  ],
})
export class HeatConfirmationPage {
  private readonly router = inject(Router);

  readonly heatPayload = input<HeatConfirmationPayload | null>(null);

  readonly judge = computed(() => this.heatPayload()?.judge ?? { id: '', name: '' });

    readonly tabs: TabOption[] = [{ value: 'teams', label: 'Equipos' }];

  searchQuery = signal<string>('');

  readonly heat = computed<HeatConfirmationHeat | null>(
    () => this.heatPayload()?.heat ?? null,
  );

  private readonly teams = computed<HeatConfirmationAthlete[]>(
    () => this.heatPayload()?.athletes.filter((a) => a.type === 'team') ?? [],
  );

  filteredAthletes = computed<HeatConfirmationAthlete[]>(() => {
    const query = this.searchQuery().toLowerCase().trim();
    const source = this.teams();
    if (!query) return source;
    return source.filter(
      (a) =>
        a.name.toLowerCase().includes(query) || a.bibNumber.includes(query),
    );
  });

  groupedAthletes = computed<
    { label: string; athletes: HeatConfirmationAthlete[] }[]
  >(() => {
    const map = new Map<string, HeatConfirmationAthlete[]>();
    for (const athlete of this.filteredAthletes()) {
      const key = `${athlete.categoryLabel} ${athlete.categoryDetail}`;
      if (!map.has(key)) map.set(key, []);
      const group = map.get(key);
      if (group) group.push(athlete);
    }
    return Array.from(map.entries()).map(([label, athletes]) => ({
      label,
      athletes,
    }));
  });

  onAthleteClick(athlete: HeatConfirmationAthlete): void {
    const heat = this.heat();
    if (!heat) return;
    if (athlete.scored) {
      void this.router.navigate(['/scoring', athlete.id, 'summary'], {
        queryParams: { readonly: 'true' },
      });
      return;
    }
    void this.router.navigate(['/heat-confirmation-summary'], {
      queryParams: {
        heatCode: heat.code,
        athleteId: athlete.id,
      },
    });
  }

  onSearch(event: Event): void {
    this.searchQuery.set((event.target as HTMLInputElement).value);
  }

  onBack(): void {
    this.router.navigate(['/heat-access']);
  }

  getTeamMembers(athlete: HeatConfirmationAthlete): { name: string }[] {
    return athlete.teamMembers?.map((n) => ({ name: n })) ?? [];
  }
}
